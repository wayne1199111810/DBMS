...
bool Btree::insert(VALUETYPE value) {
    ...
    while(inner)
    {
        int index = inner->find_value_gt(value);
        current = inner->children[index];
        inner = dynamic_cast<Bnode_inner*>(current);
    }
    Bnode_leaf* leaf = dynamic_cast<Bnode_leaf*>(current);
    if(!leaf->parent) // leaf == root
    {
        leaf->parent = parent= new Bnode_inner();
        leaf->parent->insert(leaf, 0);
    }
    if(leaf->getNumValues() == BTREE_LEAF_SIZE - 1){
        leaf->insert(value);
    }else if(leaf->parent->getNumChildren() < BTREE_FANOUT - 1){
        leaf->parent->insert(leaf->split(value), leaf->parent->getNumChildren());
    }
    return true;
}
